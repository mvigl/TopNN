...
parser.add_argument('--savebatches',  action='store_true', help='savebatches', default=False)
parser.add_argument('--M1', help='data',default=-1)
parser.add_argument('--M2', help='data',default=-1)
...
with h5py.File("/raven//u/mvigl/Stop/run/pre/SPANet_all_8_cat_final/spanet_inputs_full.h5",'r') as h5fw :
...
def run_and_save_in_batches(model_path, Momenta_data,Momenta_mask,Met_data,Met_mask, batch_size, masses,masses_slice):
    ort_sess = ort.InferenceSession(model_path)
    ...
    for i in range(num_batches):
        ...
        for j,mass in enumerate(masses):
            ...
            outputs[mass] = batch_outputs[4]
        with open(f'evals_all_SB_param_batch_{i}.pkl', 'wb') as pickle_file:
            pickle.dump(outputs, pickle_file)    
    return outputs

def run_in_batches_mass(model_path, Momenta_data,Momenta_mask,Met_data,Met_mask, batch_size, mass,mass_slice):
    ort_sess = ort.InferenceSession(model_path)
    ...
    for i in range(num_batches):
        ...
        (batch_inputs['Met_data'][:,:,-2:])[np.sum(Met_data[start_idx:end_idx,:,-2:]==[-1,-1],axis=-1)==2]=mass_slice
        batch_outputs = ort_sess.run(None, {'Momenta_data': batch_inputs['Momenta_data'],
                          'Momenta_mask': batch_inputs['Momenta_mask'],
                          'Met_data': batch_inputs['Met_data'],
                          'Met_mask': batch_inputs['Met_mask']})
    
        if i == 0: outputs = batch_outputs[4]
        else: outputs=np.concatenate((outputs,batch_outputs[4]),axis=0)
    with open(f'evals_SB_{mass}.pkl', 'wb') as pickle_file:
        pickle.dump(outputs, pickle_file)    
    return outputs
...
if __name__ == "__main__":
    ...
    if args.M1 != -1: 
        masses = f'{args.M1}_{args.M2}'
        masses_slice = [args.M1,args.M2]
        outputs = run_in_batches_mass("/raven/u/mvigl/TopReco/SPANet/spanet_param_weights_log_norm.onnx", Momenta_data,Momenta_mask,Met_data,Met_mask,batch_size,masses,masses_slice)
    elif args.savebatches: outputs = run_and_save_in_batches("/raven/u/mvigl/TopReco/SPANet/spanet_param_weights_log_norm.onnx", Momenta_data,Momenta_mask,Met_data,Met_mask,batch_size,masses,masses_slice)
    else: outputs = run_in_batches("/raven/u/mvigl/TopReco/SPANet/spanet_param_weights_log_norm.onnx", Momenta_data,Momenta_mask,Met_data,Met_mask,batch_size,masses,masses_slice)
